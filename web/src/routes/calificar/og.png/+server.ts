...
export const GET: RequestHandler = async ({ url }) => {
  const codigoMateria = url.searchParams.get("codigo_materia")!;
  const nombreMateria = decodeURIComponent(url.searchParams.get("nombre_materia")!);
  const nombreDocente = decodeURIComponent(url.searchParams.get("nombre_docente")!);
  ...
  return new ImageResponse(
    ...
    {
      materia: { codigo: codigoMateria, nombre: nombreMateria },
