...
export const load = (async () => {
	...
	for (const codigo of CODIGOS_COMENTARIOS) {
		const request = prisma.comentario.findUniqueOrThrow({
			where: {
				codigo
			},
			include: {
		...
		db_requests.push(request);
	}

	const comentarios = await Promise.all(db_requests);
