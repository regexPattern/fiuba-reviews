import { pgTable, foreignKey, pgPolicy, serial, uuid, text, boolean, timestamp, smallint, check, doublePrecision, unique, integer, primaryKey } from "drizzle-orm/pg-core"
...
export const comentario = pgTable("comentario", {
	...
	fechaCreacion: timestamp("fecha_creacion", { withTimezone: true, mode: 'string' }).default(sql`(now() AT TIME ZONE 'America/Argentina/Buenos_Aires'::text)`),
}, (table) => [
	foreignKey({
...
export const calificacionDolly = pgTable("calificacion_dolly", {
	codigo: serial().primaryKey().notNull(),
	codigoDocente: uuid("codigo_docente").notNull(),
...
export const docente = pgTable("docente", {
	...
	unique("docente_nombre_codigo_materia_key").on(table.nombre, table.codigoMateria),
	pgPolicy("Enable read access for all users", { as: "permissive", for: "select", to: ["public"], using: sql`true` }),
]);

export const equivalencia = pgTable("equivalencia", {
	...
	primaryKey({ columns: [table.codigoMateriaPlanVigente, table.codigoMateriaPlanAnterior], name: "equivalencia_pkey"}),
	pgPolicy("Enable read access for all users", { as: "permissive", for: "select", to: ["public"], using: sql`true` }),
	check("equivalencia_check", sql`codigo_materia_plan_vigente <> codigo_materia_plan_anterior`),
]);

export const catedraDocente = pgTable("catedra_docente", {
	...
	primaryKey({ columns: [table.codigoCatedra, table.codigoDocente], name: "catedra_docente_pkey"}),
	pgPolicy("Enable read access for all users", { as: "permissive", for: "select", to: ["public"], using: sql`true` }),
]);
